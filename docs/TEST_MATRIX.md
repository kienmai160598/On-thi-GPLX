...
| License-scoped mastery | `correctCount(in:)` counts only the passed questions answered correctly, so the Home progress card reflects the active license pool (B1 subset vs B2 full set) rather than the whole bank | yes | no | no | no | implemented | `ProgressStoreTests.testCorrectCountInQuestionsCountsOnlyCorrectAnswers`, `…testCorrectCountInQuestionsIgnoresUnlistedQuestions` |
| Exam countdown | `daysUntilExam` is nil when unset, the day count when ahead, 0 on exam day, and nil once the date has passed — so the countdown chip hides instead of pinning to "Hôm nay thi!" | yes | no | no | no | implemented | `ProgressStoreTests.testDaysUntilExamHidesAfterExamDatePasses` |
