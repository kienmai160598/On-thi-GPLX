...
/// on a near-black card with a gold accent — used for hero CTAs (sample exam,
/// focus situation, recommended topic). This is the one card allowed a soft
/// shadow; ordinary cards stay flat (see `GlassCard`).
struct FeatureCard: View {
    let eyebrow: String
    let title: String
    var tags: [String] = []
    /// Render the last tag as a solid gold pill (the design's highlighted pill).
    ...
    private var gold: Color { .appWarning }
                    ...
                    HStack(spacing: 6) {
                        ForEach(Array(tags.enumerated()), id: \.offset) { index, tag in
                            ...
                            Text(tag)
                                ...
                                .foregroundStyle(highlighted ? cardInk : .white)
                                .padding(.horizontal, 10)
                                .padding(.vertical, 4)
                                .background(
                                    highlighted ? gold : Color.white.opacity(0.12),
                                    in: Capsule()
                                )
                        }
                    }
